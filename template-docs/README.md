# Spring Boot Multi-Module Project Documentation

Welcome to the documentation for the Spring Boot Multi-Module Project Template. This documentation provides comprehensive information about the project structure, setup, development workflow, and customization options.

## Table of Contents

1. [Getting Started](./getting-started/README.md)

   - [Prerequisites](./getting-started/prerequisites.md)
   - [Installation](./getting-started/installation.md)
   - [Quick Start Guide](./getting-started/quick-start.md)

2. [Project Architecture](./architecture/README.md)

   - [Module Structure](./architecture/module-structure.md)
   - [Dependency Management](./architecture/dependency-management.md)
   - [Spring Modulith](./architecture/spring-modulith.md)

3. [Development Workflow](./development/README.md)

   - [Local Development](./development/local-development.md)
   - [Testing Strategy](./development/testing.md)
   - [CI/CD Integration](./development/ci-cd.md)

4. [GitHub Template Features](./template/README.md)

   - [Issue Templates](./template/issue-templates.md)
   - [GitHub Actions](./template/github-actions.md)
   - [Community Health Files](./template/community-health.md)

5. [API Documentation](./api/README.md)

   - [REST API Guidelines](./api/rest-guidelines.md)
   - [API Versioning](./api/versioning.md)

6. [Customization Guide](./customization/README.md)
   - [Adding New Modules](./customization/adding-modules.md)
   - [Configuration Options](./customization/configuration.md)

## About This Documentation

This documentation is designed to help developers quickly understand and utilize the Spring Boot Multi-Module Project Template. Whether you're new to Spring Boot or an experienced developer, you'll find valuable information to help you get started and make the most of this template.
