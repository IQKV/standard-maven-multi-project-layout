...
# Copy root Maven configuration for dependency caching
COPY pom.xml .
COPY servicename/pom.xml servicename/pom.xml

# Download dependencies in separate layer for better caching
RUN mvn dependency:go-offline -B

# Copy source code
COPY servicename/src servicename/src

# Build the application
RUN mvn clean package -DskipTests -B && \
    mkdir -p servicename/target/dependency && \
    cd servicename/target && \
...
COPY --from=builder --chown=appuser:appuser /app/servicename/target/dependency/dependencies/ ./
COPY --from=builder --chown=appuser:appuser /app/servicename/target/dependency/spring-boot-loader/ ./
COPY --from=builder --chown=appuser:appuser /app/servicename/target/dependency/snapshot-dependencies/ ./
COPY --from=builder --chown=appuser:appuser /app/servicename/target/dependency/application/ ./
